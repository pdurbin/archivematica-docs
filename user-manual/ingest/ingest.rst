   ...
   **Metadata-only re-ingest**
   ...
   1. Add or update your metadata *before* you approve Normalization to ensure 
      ...
      B. Add Metadata files
         ...
         d. Select a ``metadata.csv`` file.  Note that the structure and name of
            the file must be exactly ``metadata.csv``.  The file must be 
            staged in the same location as for Transfer Source.
...
3. The Storage Service will confirm that the AIP has been sent to the pipeline 
   for re-ingest. Proceed to the Transfer or Ingest tab of your pipeline, and follow 
   steps 3-6 above.
